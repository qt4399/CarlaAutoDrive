...
void Visualizer::drawButtons() {
    ...
    if(resetButton2.is_open==true){
        ...
        if(resetButton7.is_clicked==1){SDL_SetRenderDrawColor(renderer,130,130,130,255);}
        else if (resetButton7.is_hovered==1){SDL_SetRenderDrawColor(renderer,60,100,100,255);}
        else{SDL_SetRenderDrawColor(renderer,20,160,160,255);}
            SDL_Rect buttonRect7 = {
            resetButton7.x, resetButton7.y,
            resetButton7.width, resetButton7.height
        };
        SDL_RenderFillRect(renderer, &buttonRect7);  // 填充按钮
        if(resetButton8.is_clicked==1){SDL_SetRenderDrawColor(renderer,100,50,50,255);}
        else if (resetButton8.is_hovered==1){SDL_SetRenderDrawColor(renderer,150,50,50,255);}
        else{SDL_SetRenderDrawColor(renderer,200,50,50,255);}
            SDL_Rect buttonRect8 = {
            resetButton8.x, resetButton8.y,
            resetButton8.width, resetButton8.height
        };
        SDL_RenderFillRect(renderer, &buttonRect8);  // 填充按钮
    ...
    drawText(resetButton7.x, resetButton7.y+20, resetButton7.text);
    drawText(resetButton8.x, resetButton8.y+20, resetButton8.text);
    };
}
...
bool Visualizer::handleEvents() {
    ...
    resetButton7.is_hovered = (
        mouseX >= resetButton7.x && mouseX <= resetButton7.x + resetButton7.width &&
        mouseY >= resetButton7.y && mouseY <= resetButton7.y + resetButton7.height
    );
    resetButton8.is_hovered = (
        mouseX >= resetButton8.x && mouseX <= resetButton8.x + resetButton8.width &&
        mouseY >= resetButton8.y && mouseY <= resetButton8.y + resetButton8.height
    );
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT) {
            ...
            if (resetButton7.is_hovered) {
                resetButton7.is_clicked = true;
                ego_car.AutoDriveOn=true;
            }
            if (resetButton8.is_hovered) {
                resetButton8.is_clicked = true;
                ego_car.AutoDriveOn=false;
            }
            if (resetButton2.is_open==true){
        ...
        if (event.type == SDL_MOUSEBUTTONUP && event.button.button == SDL_BUTTON_LEFT) {
            ...
            resetButton2.is_clicked = false;
            resetButton3.is_clicked = false;
            resetButton4.is_clicked = false;
            resetButton5.is_clicked = false;
            resetButton6.is_clicked = false;
            resetButton7.is_clicked = false;
            resetButton8.is_clicked = false;
            if (resetButton5.is_hovered) {
